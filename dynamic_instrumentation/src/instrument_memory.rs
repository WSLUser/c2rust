...
use rustc_middle::mir::{
    BasicBlock, BasicBlockData, BorrowKind, Body, CastKind, Constant, Local, LocalDecl, Location, Mutability,
    Operand, Place, PlaceElem, ProjectionElem, Rvalue, SourceInfo, SourceScopeData, Statement,
...
impl<'a, 'tcx: 'a> Visitor<'tcx> for CollectFunctionInstrumentationPoints<'a, 'tcx> {
    fn visit_place(&mut self, place: &Place<'tcx>, context: PlaceContext, location: Location) {
        ...
        // Instrument each Field projection
                ...
                match elem {
                    PlaceElem::Field(field, _) => {
                        ...
                        self.add_instrumentation_point(
                            ...
                            field_fn,
                            vec![
                                InstrumentationOperand::Place(Operand::Copy(base.local.into())),
            ...
            if place.is_indirect() {
                if context.is_mutating_use() {
                    ...
                    self.add_instrumentation_point(
                        location,
                        store_fn,
                        vec![InstrumentationOperand::Place(Operand::Copy(dest))],
                        false,
                        false,
                        EventMetadata {
                            source: Some(to_mir_place(&place)),
                    ...
                    self.add_instrumentation_point(
                        location.clone(),
                        load_fn,
                        vec![InstrumentationOperand::Place(Operand::Copy(place.local.into()))],
                        false,
                        false,
                        EventMetadata {
                            source: Some(to_mir_place(&place)),
            ...
            if dest.is_indirect() {
                ...
                if let Rvalue::Use(p) = value {
                    ...
                    self.add_instrumentation_point(
                        location,
                        addr_local_fn,
                        vec![InstrumentationOperand::Place(Operand::Copy(dest)), InstrumentationOperand::AddressUsize(make_const(self.tcx, p.local.as_u32()))],
                        false,
                        false,
                        EventMetadata {
                            source: Some(to_mir_place(p)),
...
fn apply_instrumentation<'tcx>(
    ...
    for point in points {
        ...
        args.insert(0, InstrumentationOperand::AddressUsize(make_const(tcx, loc_idx)));
        ...
        if after_call {
            ...
            } = &mut call.kind
            {
                ...
                InstrumentationOperand::Place(Operand::Copy(*place))
            } else {
                panic!(
...
fn cast_ptr_to_usize<'tcx>(
    ...
    let ptr = match arg {
        ...
        InstrumentationOperand::AddressUsize(arg) => {
            return None;
        },
